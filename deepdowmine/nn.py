...
from .layers.allocate import NumericalMarkowitzWithShorting, ThesisMarkowitzFullOpti, ThesisMarkowitzMinVar
...
class LstmNetFullOpti3(torch.nn.Module, Benchmark):  
...
class LstmNetMinVar3(torch.nn.Module, Benchmark):  
    def __init__(self, n_assets, p, shrinkage_strategy, max_weight):
        super().__init__()
    ...
    def forward(self, x):
        ...
        output, hidden = self.transform_layer(x) # hidden = (hidden, cell)
...
class LstmNetFullOpti2(torch.nn.Module, Benchmark):  
    def __init__(self, n_assets, p, shrinkage_strategy, max_weight):
        super().__init__()
...
class ConvNetFullOpti2(torch.nn.Module, Benchmark):
    ...
    def forward(self, x):
        ...
        covmat_sqrt = self.covariance_layer(x_cov)
...
class ConvNetMinVar2(torch.nn.Module, Benchmark):
    def __init__(self, n_channels, lookback, n_assets, p, max_weight):
        self._hparams = locals().copy()
        super().__init__()
...
class DenseNetMinVar2(torch.nn.Module, Benchmark):
    ...
    def forward(self, x):
        ...
        covmat = self.covariance_layer(y)  # <--- actually sqrt of covmat, see https://github.com/cvxgrp/cvxpylayers/issues/136 for the context. 
...
class DenseNetFullOpti2(torch.nn.Module, Benchmark):
    ...
    def forward(self, x):
        ...
        # weights
        weights = self.portfolio_opt_layer(
            exp_rets, covmat, gamma_all, alpha_all
        )
